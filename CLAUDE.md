...
- Two dry-cycle methods, selected via the `coil_dry_method` substitution:
  ...
  - `"cool_thermo_off"` — stay in `COOL` with the setpoint bumped to `coil_dry_target_c` (default `"30"`) instead, so a shared MXZ multi-zone outdoor unit isn't disrupted for sibling zones still calling for cooling. The real setpoint is saved before the bump and restored when the cycle finishes normally; if a new command cancels it early, the setpoint is *not* automatically restored (known limitation). Unverified against real multi-zone hardware — see [#658 discussion](https://github.com/echavet/MitsubishiCN105ESPHome/issues/658#issuecomment-5140807883).
